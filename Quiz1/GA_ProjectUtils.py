...
#this will build a default dictionary of items, where the key is the item name and value is tuple of item weight and value
#per line, idx 0 is string name, idx 1 is integer weight, idx 2 is integer value
...
#used locally
def _compareTwoMSTs(MST_1, lbl1, MST_2, lbl2, args):
    wt1 = round(findTotalWeightOfMst(MST_1), 12)
    wt2 = round(findTotalWeightOfMst(MST_2), 12)
    if(abs(wt1 - wt2) < 1e-12):
        if args.autograde == 1:
            print("Correct Kruskal's implementation : {} Weight : {} {} Wt : {} ".format(lbl1, wt1, lbl2, wt2))
        return True
    else:
        diff12 = MST_1 - MST_2
        ...
        if args.autograde == 1:
            print("Errors in Kruskal's implementation :")
            print("\t{} Weight : {} {} Wt : {}".format(lbl1, wt1, lbl2, wt2))
        return False
...
def verify_MSTKruskalResults(args, MST_Kruskal, printMST=False):
    ...
    return _compareTwoMSTs(MST_Kruskal,"Kruskal's Result", MST_Correct, "Correct Result", args)
...
class findX():
    ...
    def start(self, seed):
        random.seed(seed)
        self.__lookupCount=0
        self.__n = random.randint(5000, 10000)
        self.__A = random.sample(range(50000), self.__n+1) # sample extra value to avoid A[n] error
        self.__A.sort()
        self.x = self.__A[random.randint(1,self.__n)]
        self.__maxCalls = int(math.log(self.__n, 2)*3) + 2 
        return self.x
